...
4.times do |log|
	AuditLog.create!(user_id: @second_user.id,status: 0,start_date: (Chronic.parse('monday', :context => :past) - (7*log).days))
	
end 
3.times do |log|
	AuditLog.create!(user_id: @user.id,status: 0,start_date: (Chronic.parse('monday', :context => :past) - (7*log).days))
